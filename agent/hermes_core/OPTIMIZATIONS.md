...
> 本文记录 vendor 自 desk-pet 后，在 Assa 侧所做的适配与已知边界。
...
| Ollama 向量 | `OllamaEmbedder`（默认 `bge-m3`，`ASSA_EMBED=local_hash` 可关）；启动探活，失败回退 `LocalHashEmbedder` |
...
| 变量 | 默认 | 含义 |
|---|---|---|
| `ASSA_EMBED` | `ollama` | `local_hash` 强制离线哈希向量 |
| `ASSA_EMBED_MODEL` | `bge-m3` | Ollama embedding 模型 |
| `ASSA_OLLAMA_URL` | `http://127.0.0.1:11434` | Ollama 地址 |
| `ASSA_EMBED_TIMEOUT` | `8` | 单次 embed 超时（秒） |
