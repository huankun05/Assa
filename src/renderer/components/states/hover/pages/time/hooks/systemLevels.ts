 ...
 * 预热延迟（ms）：避开应用启动高峰。
 *
 * 预热会触发主进程拉起 .NET helper 常驻进程（冷启动要 JIT + 加载运行时，
 * 启动期与 Vite 编译、窗口创建抢 CPU 会让界面发涩）。亮度/音量面板不可能在
 * 启动头几秒就被打开，推迟到空闲后预热，用户无感知；真提前打开时
 * hooks 的 ensureXxxWarm() 会立即兜底拉一次。
 */
const WARM_DELAY_MS = 4000;

/**
 * 启动预热：只拉取一次真值（推送链路建立前的兜底）。幂等。
