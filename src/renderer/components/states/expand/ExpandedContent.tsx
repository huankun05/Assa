...
export function ExpandedContent(): React.ReactElement {
  ...
  const handleSetMaxExpand = useCallback((): void => {
    ...
    const activeTabVisible = maxExpandNavLayoutConfig.some((item) => item.id === maxExpandTab && item.visible);
    if (!activeTabVisible) {
      setMaxExpandTab(targetMaxExpandTab);
    }
    preloadEagerWhenPerformanceModeDisabled();
    setMaxExpand();
  }, [firstVisibleMaxExpandTab, hasAvailableMaxExpandTab, maxExpandNavLayoutConfig, maxExpandTab, preloadEagerWhenPerformanceModeDisabled, setMaxExpand, setMaxExpandTab]);
