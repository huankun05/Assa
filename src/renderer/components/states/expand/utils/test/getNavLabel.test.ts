...
describe('getNavLabel', () => {
  ...
  describe('return value', () => {
    ...
    it('should return a different value when t returns differently per call', () => {
      ...
      expect(getNavLabel('tools', t)).toBe('second')
    })
  })
  ...
  describe('i18n key format', () => {
    it('should always prefix the key with "expanded.nav."', () => {
      const t = createMockT()
      const tabs = ['hover', 'tools', 'translation', 'performanceMonitor', 'maxExpand'] as const
      tabs.forEach((tab) => {
        t.mockClear()
        getNavLabel(tab, t)
    ...
    it('should embed the tab ID directly after the prefix', () => {
      const t = createMockT()
      getNavLabel('tools', t)
      expect(t.mock.calls[0][0]).toBe('expanded.nav.tools')
    })
  })
})
