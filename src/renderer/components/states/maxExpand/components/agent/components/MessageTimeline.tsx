...
const MessageTimeline = React.memo(function MessageTimeline({
  ...
  const todoSnapshots: AiTodoSnapshot[] = useMemo(() => {
    return Array.isArray(msg.todoSnapshots) ? msg.todoSnapshots : [];
  }, [msg.todoSnapshots]);
  ...
  const unturnedTodoSnapshots = useMemo(() => todoSnapshots.filter((snap) => !(snap.turn > 0)), [todoSnapshots]);
