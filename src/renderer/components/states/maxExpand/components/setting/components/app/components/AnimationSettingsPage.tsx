...
export function AnimationSettingsPage(): ReactElement {
  ...
  const [startupResetHint, setStartupResetHint] = useState(false);
  ...
  /**
   * 重置为首次启动：删除首次启动标记，使下次启动重新播放启动动画并进入引导
   * @description 启动动画与引导都只在首次启动出现，需要一个入口可以重新触发该体验
   */
  const handleResetStartupExperience = (): void => {
    window.api.resetGuide().then((ok) => {
      if (!ok) return;
      setStartupResetHint(true);
      window.setTimeout(() => setStartupResetHint(false), 3000);
    }).catch(() => {});
  };
          ...
          <div className="settings-splash-preview-cards">
            <div className="settings-card">
              <div className="settings-card-header">
                ...
                <div className="settings-card-subtitle">{t('settings.app.animation.startupAnimationHint', { defaultValue: '开启后仅首次启动显示启动动画（画面右下角可直接进设置），关闭后永不显示' })}</div>
              </div>
              <div className="settings-card-inline-row">
                <label className="settings-card-check">
                  <input
                ...
                <button
                  ...
                  onClick={handleResetStartupExperience}
                >
                  {t('settings.app.animation.resetStartupExperience', { defaultValue: '重置为首次启动' })}
                </button>
              </div>
              {startupResetHint && (
                <div className="settings-card-hint-text">
                  {t('settings.app.animation.resetStartupExperienceHint', { defaultValue: '已重置，下次启动将重新播放启动动画并进入首次引导' })}
                </div>
              )}
            </div>
            <div className="settings-card">
