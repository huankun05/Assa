...
import { SEARCHABLE_SETTINGS, SETTINGS_TAB_ICONS, type SearchableSettingItem, type AppSettingsPageKey, type MusicSettingsPageKey, type NetworkSettingsPageKey, type SettingsSidebarTabKey } from '../../utils/settingsConfig';
    ...
    <div className="max-expand-settings-section settings-index-section">
      <div className="settings-index-header">
        <div className="max-expand-settings-title">
          ...
          <button className="settings-card-action-btn" type="button" onClick={resetNavConfig}>{t('settings.index.reset', { defaultValue: '恢复默认' })}</button>
          <button
            className={`settings-lyrics-source-btn ${navEditMode ? 'active' : ''}`}
            type="button"
            onClick={() => {
              if (navEditMode) {
