        ...
        {mailAccounts.map((account) => (
          <div key={account.id} className={`settings-mail-account-tab ${account.id === (activeAccount?.id ?? '') ? 'active' : ''}`}>
            ...
            {mailAccounts.length > 1 && (
              <button type="button" className="settings-lyrics-source-btn" onClick={() => removeAccount(account.id)} title={t('settings.mail.accounts.remove', { defaultValue: '移除' })} aria-label={t('settings.mail.accounts.remove', { defaultValue: '移除' })}>×</button>
            )}
          </div>
        ))}
        {mailAccounts.length < MAX_MAIL_ACCOUNTS && (
          <button type="button" className="settings-lyrics-source-btn" onClick={addAccount} title={t('settings.mail.accounts.add', { defaultValue: '添加账户' })} aria-label={t('settings.mail.accounts.add', { defaultValue: '添加账户' })}>+</button>
        )}
          </div>
