...
export function SettingsTab(): ReactElement {
  ...
  const { fetchWeatherData, setNotification } = useIslandStore();
  ...
  const translatedMusicSettingsPageLabels = useMemo<Record<MusicSettingsPageKey, string>>(() => ({
    ...
    like: t('settings.musicPages.like', { defaultValue: MUSIC_SETTINGS_PAGE_LABELS.like }),
  }), [t]);
  ...
  useEffect(() => {
    ...
    window.api.navOrderGet().then((navConfig) => {
      ...
      if (visibleRaw.length > 0 || hiddenRaw.length > 0) {
        ...
        const ensuredVisible = [...validVisible];
  ...
  const handleAddWhitelist = (): void => {
    ...
    // 浏览器网页播放不在本地播放器识别范畴，即使手动加入也会在主进程被忽略
    if (isBrowserSource(nextItem)) {
      setWhitelistDraft('');
      setWhitelistInputError(t('settings.music.whitelist.browserNotSupported', { defaultValue: '暂不支持浏览器网页播放' }));
      return;
    }
