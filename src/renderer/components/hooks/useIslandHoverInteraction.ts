...
export function useIslandHoverInteraction(options: UseIslandHoverInteractionOptions): void {
  ...
  useEffect(() => {
    ...
    const checkMousePosition = async (): Promise<void> => {
      ...
      if (state === 'notification' || state === 'agent' || state === 'stt' || state === 'cli' || state === 'agentVoiceInput' || state === 'guide' || state === 'announcement') {
        if (inWindow) {
          setMousePassthrough(false);
        }
        scheduleCheck();
      ...
      if (inWindow) {
        ...
        if (isHoveringRef.current && leaveTimerRef.current === null) {
          const shouldLeave = state === 'expanded' ? expandLeaveIdleRef.current
