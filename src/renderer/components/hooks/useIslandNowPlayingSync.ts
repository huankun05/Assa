...
export function useIslandNowPlayingSync(options: UseIslandNowPlayingSyncOptions): void {
  ...
  useEffect(() => {
    ...
    const processInfo = (info: NowPlayingInfo | null) => {
      ...
      if (newKey && newKey !== songKeyRef.current) {
        ...
        const loadLyrics = async (): Promise<void> => {
          ...
          const normalLyricsPromise = fetchLyricsWithTranslation(title, artist, deviceId, info!.songId)
            .catch(() => null);
    ...
    const unsubscribe = window.api?.onNowPlayingInfo(processInfo);

    // 监听器挂载完成后，通知主进程主动补发一次当前播放信息，规避启动阶段推送早于挂载导致的漏播
    window.api?.notifyNowPlayingReady();
