...
export async function getAssaVisibleName(): Promise<string> {
...
export function getAssaVisibleNameSync(): string {
  return cached?.visible_name || '汐月';
}

export async function getAssaEnglishName(): Promise<string> {
  if (cached?.english_name) return cached.english_name;
  const _ = await getAssaVisibleName();
  return cached?.english_name || 'Xiyue';
}

export function getAssaEnglishNameSync(): string {
  return cached?.english_name || 'Xiyue';
}

export async function getAssaModelDefault(): Promise<string> {
...
export function getAssaModelDefaultSync(): string {
  return cached?.model_default || 'qwen3-4b-32k';
}

export function invalidateAssaIdentityCache(): void {
  cached = null;
}
