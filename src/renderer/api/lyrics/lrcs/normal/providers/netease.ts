...
/**
 * 按主进程下发的 songId 直接取词（绕过二次搜索，保证与喜欢状态基于同一首歌）。
 * 命中纯音乐占位或空结果时返回 null，由上层回退到普通搜索流程。
 */
export async function fetchLyricsFromNeteaseBySongId(songId: number | string): Promise<LyricsFetchResult | null> {
  if (songId === undefined || songId === null || songId === '' || Number(songId) <= 0) return null;
  try {
    return await fetchLyricBySongId(String(songId));
  } catch {
    return null;
  }
}
