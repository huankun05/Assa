...
import { fetchLyricsWithTranslationFromNetease, fetchLyricsFromNeteaseBySongId } from './normal/providers/netease';
...
export async function fetchLyricsWithTranslation(
  ...
  // 网易云已登录时，优先用主进程下发的 songId 直接取词：
  // 与喜欢状态基于同一首歌（同一 songId），避免两套搜索各自 pickBestMatch 选中不同版本。
  if (primary === 'netease' && songId) {
    const byId = await fetchLyricsFromNeteaseBySongId(songId);
    if (byId && byId.lyrics.length > 0) return byId;
  }
...
export async function fetchLyrics(
  ...
  const result = await fetchLyricsWithTranslation(title, artist, sourceAppId, songId);
...
async function resolveSourceAppId(): Promise<string> {
  try {
    ...
    // 双保险：即便主进程放行，渲染端也过滤掉浏览器音源（脏标题会污染匹配）
    const sources = resp.sources.filter((s) => !isBrowserSource(s.sourceAppId));
    if (sources.length === 0) return '';
    const playing = sources.find((s) => s.isPlaying && s.hasTitle);
    return (playing ?? sources[0]).sourceAppId ?? '';
  } catch {
    return '';
  }
}
