 ...
 *   支持语音打断：abort / 新语音输入时 stopAssaTtsPlayback()。
 */

import type { MihtnelisAgentStreamEvent } from './types';

interface StreamAssaAgentRequest {
  message: string;
  signal?: AbortSignal;
 ...
 */
export function stopAssaTtsPlayback(): void {
  if (currentTtsAudio) {
    try {
 ...
 */
export async function streamAssaAgent(request: StreamAssaAgentRequest): Promise<void> {
  const { message, signal, onEvent } = request;
  const sessionId = `assa-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;

  /** 用户打断：停 TTS + 通知侧车 abort */
  const onAbort = (): void => {
    stopAssaTtsPlayback();
    window.api.assaStreamAbort(sessionId).catch(() => {});
  };
  ...
  try {
    const handler = (raw: unknown): void => {
      ...
      switch ((raw as { type?: string } | null)?.type) {
        ...
        case 'final':
          ...
          break;
        case 'error':
          emit(onEvent, { type: 'error', payload: { message: str(payload.message) || 'AI 返回错误' } });
          break;
        default:
          break;
      }
    ...
    const unsubscribe = window.api.onAssaStreamEvent(sessionId, handler);

    try {
      const res = await window.api.assaStreamChatStart(sessionId, message);
      if (!res?.ok) {
