...
const api = {
  ...
  /**
   * 打开倒数日/TODOs 独立窗口
   * @param tab - 可选：目标标签页，传入后主进程会先切到该标签再显示窗口
   */
  openStandaloneWindow: (tab?: string): Promise<boolean> => {
    return ipcRenderer.invoke('app:open-standalone-window', tab);
  },
  /**
   * 打开独立窗口并停留到「设置」标签
   */
  ...
  /**
   * 通知主进程：灵动岛播放监听器已就绪，主进程应主动补发一次当前播放信息。
   * @description 规避启动阶段主进程推送早于监听器挂载导致的漏播（事件驱动，非轮询）
   */
  notifyNowPlayingReady: (): void => {
    ipcRenderer.send('media:nowplaying-ready');
  },
  /** ===== 歌曲信息监听 API ===== */
  /**
   * 订阅歌曲信息变更事件
  ...
  /**
   * 打开系统计算器（按平台调出对应应用：Windows 计算器 / macOS Calculator / Linux 常见计算器）
   * @returns 无返回值
   */
  openCalculator: (): void => {
    ipcRenderer.send('system:open-calculator');
  },
  /**
   * 获取当前屏幕亮度
   ...
   */
  musicLikeSync: (title: string, artist: string, album?: string): Promise<{ liked: boolean }> => {
    return ipcRenderer.invoke('music:like:sync', title, artist, album);
  },
  /**
   * 获取「喜欢」快捷键配置
