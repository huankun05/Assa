...
export function registerMediaIpcHandlers(options: RegisterMediaIpcHandlersOptions): void {
  ipcMain.handle('media:current-info:get', () => {
    ...
    if (entry?.hasTitle) return entry.payload;
    // SMTC 无当前会话（如网易云未注册 SMTC）时返回 null，由渲染进程据其自身状态处理
    return null;
  });

  /**
   * 监听就绪通知：灵动岛播放监听器挂载完成后，主进程主动补发一次当前播放信息。
   * @description 事件驱动，避免轮询；覆盖「播放早于咱窗口就绪、推送被丢弃」的漏播场景
   */
  ipcMain.on('media:nowplaying-ready', () => {
    const entry = options.getSmtcSessionRuntime()?.get(options.getCurrentDeviceId());
  ...
  ipcMain.handle('media:play-pause', () => {
    const entry = options.getSmtcSessionRuntime()?.get(options.getCurrentDeviceId());
    // SMTC 无当前会话时不派发控制（原生 SMTC 无会话则无法控制该应用）
    if (!entry?.hasTitle) return;
    const isPlaying = Boolean((entry.payload as Record<string, unknown>)?.isPlaying);
