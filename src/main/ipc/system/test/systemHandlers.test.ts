...
describe('system ipc handlers', () => {
  ...
  beforeEach(() => {
    ...
    execMock.mockReset();
    spawnMock.mockReset();
    // spawn 默认返回带 unref 的子进程对象，模拟真实行为
  ...
  it('opens task manager event on win32', () => {
    ...
    expect(spawnMock).toHaveBeenCalledWith('taskmgr', [], { detached: true, stdio: 'ignore' });
  });

  it('does not open task manager on non-win32 and catches spawn errors', () => {
  ...
  it('opens system calculator on win32', () => {
    registerSystemIpcHandlers({
      queryRunningNonSystemProcessNames: vi.fn(async () => []),
      queryRunningNonSystemProcessesWithIcons: vi.fn(async () => []),
      queryOpenWindowsWithIcons: vi.fn(async () => []),
    ...
    onHandlers.get('system:open-calculator')?.();
    expect(spawnMock).toHaveBeenCalledWith('calc', [], { detached: true, stdio: 'ignore' });
  });

  it('opens Calculator app on darwin', () => {
    registerSystemIpcHandlers({
      queryRunningNonSystemProcessNames: vi.fn(async () => []),
      queryRunningNonSystemProcessesWithIcons: vi.fn(async () => []),
      queryOpenWindowsWithIcons: vi.fn(async () => []),
    ...
    onHandlers.get('system:open-calculator')?.();
    expect(spawnMock).toHaveBeenCalledWith('open', ['-a', 'Calculator'], { detached: true, stdio: 'ignore' });
  });

  it('falls back through calculator candidates on linux', async () => {
    registerSystemIpcHandlers({
      queryRunningNonSystemProcessNames: vi.fn(async () => []),
      queryRunningNonSystemProcessesWithIcons: vi.fn(async () => []),
      queryOpenWindowsWithIcons: vi.fn(async () => []),
    ...
    // 前两个候选不存在（触发 error fallback），第三个成功
    const createChildMock = (fail: boolean) => {
      ...
      if (fail) queueMicrotask(() => (handlers['error'] ?? []).forEach((cb) => cb()));
      return child;
    };
    spawnMock
      .mockImplementationOnce(() => createChildMock(true))
      .mockImplementationOnce(() => createChildMock(true))
      .mockImplementationOnce(() => createChildMock(false));

    onHandlers.get('system:open-calculator')?.();
    await vi.waitFor(() => {
      expect(spawnMock).toHaveBeenNthCalledWith(1, 'gnome-calculator', [], { detached: true, stdio: 'ignore' });
      expect(spawnMock).toHaveBeenNthCalledWith(2, 'kcalc', [], { detached: true, stdio: 'ignore' });
      expect(spawnMock).toHaveBeenNthCalledWith(3, 'qalculate-gtk', [], { detached: true, stdio: 'ignore' });
    });
  });
