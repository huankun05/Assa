...
export function registerSystemIpcHandlers(options: RegisterSystemIpcHandlersOptions): void {
  ipcMain.on('system:open-task-manager', () => {
    try {
      if (process.platform === 'win32') {
        // 用 spawn 直接拉起 taskmgr.exe（走 PATH 命中 System32），
        // 避免 exec 经过 shell(cmd.exe) 的额外进程启动开销，调出更跟手。
        const child = spawn('taskmgr', [], { detached: true, stdio: 'ignore' });
        child.unref();
      }
    } catch (err) {
  ...
  ipcMain.on('system:open-calculator', () => {
    try {
      if (process.platform === 'win32') {
        const child = spawn('calc', [], { detached: true, stdio: 'ignore' });
        child.unref();
      } else if (process.platform === 'darwin') {
        const child = spawn('open', ['-a', 'Calculator'], { detached: true, stdio: 'ignore' });
        child.unref();
      } else {
        // Linux 无统一默认计算器，按常见桌面环境依次尝试，找不到则静默放弃
        const candidates = ['gnome-calculator', 'kcalc', 'qalculate-gtk', 'xcalc'];
        const launch = (index: number): void => {
          if (index >= candidates.length) return;
          const child = spawn(candidates[index], [], { detached: true, stdio: 'ignore' });
      ...
      console.error('[System] open-calculator error:', err);
    }
  });
