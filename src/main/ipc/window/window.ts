...
export function registerWindowIpcHandlers(options: RegisterWindowIpcHandlersOptions): void {
  ...
  const applyWindowGeometry = (
    ...
    /** 必须先于下方赋值读取：logicalWindowSize 紧接着就会被覆盖为本次的新可见尺寸，
     *  若在其之后读取，sameVisible 会退化成「新值 === 新值」恒真，去重判断失效 */
    const prevVisible = logicalWindowSize;
    ...
    /** 去重：目标边界与当前一致时跳过原生 setBounds/setShape/invalidate，
     *  避免 hover 反复进出等场景下每帧触发原生窗口几何变更与整页重绘（卡顿根因之一）
     *  注意：两段式 resize 中可见区（shape）可能与边界分离变化，须同时比对可见尺寸。
     *  一旦此处误判命中 return，setShape 与 invalidate 会被跳过：
     *  原生裁剪区停留在上一次的尺寸，与 React 侧已切换的 CSS 尺寸失配，
     *  表现为灵动岛收起后内容显示残留/裁切异常，直到下一次几何变更才恢复 */
    const sameVisible =
      prevVisible !== null &&
      prevVisible.width === visibleWidth &&
      prevVisible.height === visibleHeight;
    if (
