...
/**
 * @file assaIdentity.ts
 * @description 汐月身份查询 IPC：从侧车或本地配置读取程序化身份信息。
 */

import { ipcMain } from 'electron';

export interface AssaIdentity {
  name: string;
  visibleName: string;
  englishName: string;
  role: string;
...
let cachedIdentity: AssaIdentity | null = null;
let cacheExpiresAt = 0;
const CACHE_TTL_MS = 30_000;

async function fetchIdentityFromSidecar(): Promise<AssaIdentity | null> {
  try {
    const port = await getAssaAgentPortSafe();
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), 5_000);
    const res = await fetch(`http://127.0.0.1:${port}/identity`, { signal: controller.signal as any });
    clearTimeout(timeout);
    if (!res.ok) return null;
    return (await res.json()) as AssaIdentity;
  } catch {
    return null;
  }
}

function getCachedIdentity(): AssaIdentity | null {
...
function setCachedIdentity(data: AssaIdentity | null): void {
  cachedIdentity = data;
...
async function getAssaAgentPortSafe(): Promise<number> {
  try {
    const { getAssaAgentPort } = await import('../../services/assaAgentService');
    return getAssaAgentPort();
  } catch {
    return 8765;
  }
}

export function registerAssaIdentityIpcHandlers(): void {
  ipcMain.handle('assa:identity', async (): Promise<AssaIdentity> => {
    ...
    const fallback: AssaIdentity = {
      name: '汐月',
      visibleName: '汐月',
      englishName: 'Assa',
      role: '本地常驻 AI 管家',
      modelDefault: 'qwen3-4b-32k',
      systemIdentifier: 'assa-local-agent',
      greeting: '我在，怎么了？',
