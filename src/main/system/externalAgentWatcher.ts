...
import { queryRunningNonSystemProcessNames, normalizeProcessName } from './runningProcesses';
...
export function createExternalAgentWatcher(options: CreateExternalAgentWatcherOptions): ExternalAgentWatcherService {
  ...
  async function checkNow(): Promise<void> {
    ...
    try {
      ...
      // 一次枚举系统进程，再内存匹配所有受监控的 Agent 进程名。
      // 此前每个进程名各 spawn 一次 tasklist（每 4 秒最多 9 次），
      // 高频枚举进程会频繁打断主进程事件循环，导致鼠标卡顿。
      const runningNames = new Set(
        (await queryRunningNonSystemProcessNames()).map(normalizeProcessName),
      );

      AGENT_PROCESS_NAMES.forEach((processName) => {
        const running = runningNames.has(normalizeProcessName(processName));
