...
/** 轻量窗口标题信息（不含图标） */
export interface OpenWindowTitleInfo {
  processName: string;
  title: string;
}

/**
 * 查询当前打开窗口的标题（轻量版，不提取图标）
 * @description 仅供窗口标题轮询类场景使用：openWindows 一次调用即返回，
 *              不做图标提取（图标提取开销大且对提权进程可能失败）
 * @returns 窗口标题信息数组
 */
export async function queryOpenWindowTitles(): Promise<OpenWindowTitleInfo[]> {
  ...
  try {
    ...
    console.error('[Window] query open window titles failed:', err);
    return [];
  }

  return windows
    .map((windowInfo): OpenWindowTitleInfo | null => {
      const title = typeof windowInfo.title === 'string' ? windowInfo.title.trim() : '';
      if (!title) return null;
      const processName = typeof windowInfo.owner?.name === 'string' ? windowInfo.owner.name.trim() : '';
      return { processName, title };
    })
    .filter((item): item is OpenWindowTitleInfo => Boolean(item));
}
