...
const { appendFileSyncMock, existsSyncMock, mkdirSyncMock, readFileSyncMock } = vi.hoisted(() => {
  ...
  const schemaPath = nodePath.join(process.cwd(), 'schemas', 'assa_tools.json');
  return {
    appendFileSyncMock: vi.fn(),
    existsSyncMock: vi.fn((p: string) => String(p).includes('assa_tools.json') || String(p).includes('logs')),
    mkdirSyncMock: vi.fn(),
...
describe('assaFinalCheck', () => {
  it('rejects empty and unregistered tools', () => {
    expect(assaFinalCheck({ tool: '' }).allowed).toBe(false);
    expect(assaFinalCheck({ tool: 'no.such.tool' })).toMatchObject({ allowed: false, denyReason: '未注册工具：no.such.tool' });
  });

  it('allows read-only tools without confirmation or workspace', () => {
    expect(assaFinalCheck({ tool: 'file.read' })).toEqual({ allowed: true });
    expect(assaFinalCheck({ tool: 'sys.info' })).toEqual({ allowed: true });
  });

  it('requires workspace for write-class tools when empty', () => {
    expect(assaFinalCheck({ tool: 'file.write', workspaces: [] }).allowed).toBe(false);
    expect(assaFinalCheck({ tool: 'file.write', workspaces: ['D:\\ws'] }).allowed).toBe(true);
  });

  it('enforces deny/ask minimal set: confirm tools need userConfirmed', () => {
    const noConfirm = assaFinalCheck({ tool: 'file.delete', workspaces: ['D:\\ws'] });
    expect(noConfirm.allowed).toBe(false);
    expect(noConfirm.requiresConfirmation).toBe(true);

    expect(assaFinalCheck({ tool: 'file.delete', workspaces: ['D:\\ws'], userConfirmed: true })).toEqual({ allowed: true });
    expect(assaFinalCheck({ tool: 'cmd.exec', workspaces: ['D:\\ws'] }).requiresConfirmation).toBe(true);
    expect(assaFinalCheck({ tool: 'cmd.exec', workspaces: ['D:\\ws'], userConfirmed: true }).allowed).toBe(true);
  });

  it('workspace check runs before confirmation check', () => {
    const r = assaFinalCheck({ tool: 'file.delete', workspaces: [], userConfirmed: true });
    expect(r.allowed).toBe(false);
    expect(r.requiresConfirmation).toBeUndefined();
  });

  it('every confirm:true tool in the allowlist is gated', () => {
    for (const meta of listAssaTools()) {
      if (!meta.confirm) continue;
      const r = assaFinalCheck({ tool: meta.id, workspaces: ['D:\\ws'] });
      expect(r.requiresConfirmation, meta.id).toBe(true);
    }
  });
});

describe('normalizeAssaToolRequest', () => {
  it('lower-cases tool, objectifies args, stringifies workspaces, coerces userConfirmed', () => {
    expect(normalizeAssaToolRequest({ tool: '  File.Read ', arguments: [1], workspaces: [1, ' a ', ''], userConfirmed: 'yes' })).toEqual({
      tool: 'file.read',
      arguments: {},
      workspaces: ['1', 'a'],
      userConfirmed: false,
    });
    expect(normalizeAssaToolRequest({ tool: 'x', userConfirmed: true }).userConfirmed).toBe(true);
  });
});

describe('assaExecuteTool', () => {
  beforeEach(() => {
  ...
  it('audits denial without calling impl', async () => {
    const impl = vi.fn();
    const r = await assaExecuteTool({ tool: 'nope' }, impl, { source: 'test' });
  ...
  it('audits success path (the gap that only-denial logging missed)', async () => {
    ...
    const r = await assaExecuteTool({ tool: 'SYS.info', arguments: { a: 1 } }, impl, { source: 'renderer-ipc' });
  ...
  it('audits impl failure and converts thrown errors into a result', async () => {
    ...
    const r = await assaExecuteTool({ tool: 'sys.info' }, impl);
  ...
  it('denies confirm tools from callers that cannot confirm (main-process orchestrators)', async () => {
    const impl = vi.fn();
    const r = await assaExecuteTool({ tool: 'cmd.exec', arguments: { command: 'dir' }, workspaces: ['D:\\ws'] }, impl, { source: 'ollama-orchestrator' });
    expect(r.success).toBe(false);
  ...
  it('runs confirm tools once userConfirmed is present and records it in audit', async () => {
    ...
    const r = await assaExecuteTool({ tool: 'cmd.exec', arguments: { command: 'dir' }, workspaces: ['D:\\ws'], userConfirmed: true }, impl);
  ...
  it('truncates oversized strings in audit lines', async () => {
    ...
    await assaExecuteTool({ tool: 'file.read', arguments: { path: big } }, impl);
  ...
  it('never throws when audit write fails', async () => {
    ...
    const impl = vi.fn().mockResolvedValue({ success: true, result: {}, error: '', durationMs: 1 });
    await expect(assaExecuteTool({ tool: 'sys.info' }, impl)).resolves.toMatchObject({ success: true });
  });
});
