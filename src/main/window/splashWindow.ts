...
/** 创建启动画面窗口的可选项 */
interface ShowSplashWindowOptions {
  /**
   * 是否需要接收鼠标交互
   * @description 启动画面上出现可点击控件（首次启动的设置按钮）时必须为 true，
   *   否则 Windows 下窗口不可聚焦，按钮点击可能无响应
   */
  interactive?: boolean;
}
 ...
 * @param options - 创建选项，interactive 为 true 时窗口可聚焦以接收点击
 */
function showSplashWindow(options: ShowSplashWindowOptions = {}): void {
  ...
  splashWindow = new BrowserWindow({
    ...
    alwaysOnTop: true,
    skipTaskbar: true,
    focusable: options.interactive === true,
    title: '',
    hasShadow: true,
    icon: is.dev
...
/**
 * 立即淡出并关闭启动画面
 * @description 用户主动跳过启动动画（例如点击画面上的设置按钮）时调用，
 *   不等待视频播完，避免用户在设置窗口操作时画面仍滞留
 */
...
export { showSplashWindow, closeSplashWindow, dismissSplashWindow, getSplashWindow };
