...
import { createNeteaseWatcherLauncher } from './music/neteaseWatcherLauncher';
...
/** netease-watcher 常驻进程：提供网易云真实播放进度（SMTC/UIA 均不可靠时的唯一来源） */
const neteaseWatcherLauncher = createNeteaseWatcherLauncher();
...
const hotkeyService = createHotkeyService({
  ...
  onScreenshotHotkey: () => {
    // 统一入口：原生截图引擎优先（triggerScreenshot 内部已做原生优先判断），
    // 未配置或失败时回退内置 Electron 截图窗。热键与工具栏按钮行为一致。
    captureWindowService.triggerScreenshot().catch((err) => {
