...
describe('createSmtcService', () => {
  ...
  describe('worker message: session-update', () => {
    ...
    it('does not auto-assign currentDeviceId when new source is not playing and has no title', async () => {
      const { createSmtcService } = await import('../smtcService');
      const svc = createSmtcService(defaultOptions({ getWhitelist: () => ['spotify'] }));
      svc.setCurrentDeviceId('');
      svc.initWorker();
    ...
    it('auto-assigns currentDeviceId when new source is paused but has a title (startup detection)', async () => {
      const { createSmtcService } = await import('../smtcService');
      const svc = createSmtcService(defaultOptions({ getWhitelist: () => ['spotify'] }));
      svc.setCurrentDeviceId('');
      svc.initWorker();
