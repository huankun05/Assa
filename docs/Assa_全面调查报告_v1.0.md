...
> **配套子报告**：`docs/Assa_Electron_Source_Audit.md`（Electron 层）、`docs/Assa_Agent_Voice_Source_Audit.md`（Agent/语音层）、`docs/README.md`（文档索引）、`docs/Assa_Cleanup_Report_2026-09-10.md`（清理清单）
...
| 8 | 循环"无迭代预算" | **不成立**。`_MAX_TOOL_ROUNDS = 8`（assa.json `tools.max_rounds_per_turn: 8` 可配），超限后兜底再问一次。"无上下文压缩、无错误重试"仍属实 |
...
| 11 | store.py "3 条测试数据，`_DEFAULT_DB_PATH` 硬编码相对路径" | 3 条测试数据属实；但代码中**不存在** `_DEFAULT_DB_PATH`——路径经 `resolve_data_dir()` 收口（`ASSA_DATA_DIR` 环境变量 > 仓库 data/）；store.py docstring 已自述为 Phase 0 stub，将整体替换为 desk-pet 引擎 |
...
**审校新发现（v1.0 未覆盖）**：
- `assa.json` 中 `browser.allowed_domains` 域名白名单**未被任何代码强制执行**（playwright_client.py 无引用），浏览器工具实际不受域限制——建议纳入安全缺口清单
...
- 启动校验：policy.py 与 `assaToolSchema.ts` 都从同一份 schema 加载，元数据不一致拒绝启动并告警。policy.py 自己的 docstring 就要求"两份逻辑共享单一事实源、语义必须一致"，但现在两边各自硬编码。
...
| LLM | Ollama qwen3-4b-32k（非流式调用） | agent/server.py + persona/assa.json |
...
- **hover 6 个 Tab**：time（控制中心）/ lyrics（歌词+喜欢）/ weather / assa（AI 双态）/ pomodoro（番茄钟）/ expand（入口，常驻）；支持 `custom:*` 用户自定义页
...
- 本地 LLM 对话（Ollama qwen3-4b-32k，默认模型来自 persona/assa.json `model_default`）
- SSE 流式输出（**模拟流式**：完整回复后按 8 字符 + 20ms 人工切片）
- 工具化 agent 循环：19 个内联工具定义，每轮对话最多 8 轮工具调用（`_MAX_TOOL_ROUNDS`，assa.json 可配），超限兜底再问一次
- 语音输入：渲染层 getUserMedia 录音 → faster-whisper STT
- 语音输出：kokoro TTS（zf_xiaobei 音色，24k wav）+ pyttsx3 兜底
- 浏览器自动化（Playwright，6 个工具；assa.json 配置了 `browser.allowed_domains` 白名单但**代码未强制执行**）
- 双闸门安全：Python policy.py 预检 + Electron 主进程 assaFinalCheck 终审
- 身份层：assa.json 程序化身份 → identity.py → 主进程/前端统一读取
...
│   │   ├── agent/               # AI 对话/工具执行/语音（localToolIpc.ts 含 assaFinalCheck 终审）
│   │   ├── app/                 # 应用配置/更新/账户（app.ts 2599 行，含工具执行终审）
│   │   ├── media/               # 音乐/SMTC/歌词/音量（media:get/set-volume 为 stub）
│   │   ├── settings/            # 设置读写
│   │   ├── system/              # 系统控制/剪贴板/下载
│   │   └── window/              # 窗口管理/灵动岛状态（capture.ts 含长截图双引擎）
│   ├── services/                # 后台服务（SMTC/通知/更新/assaAgentService 侧车 spawn + 30s 健康巡检/assaToolSchema 终审+审计）
...
│   └── persona/                 # 人格设定（assa.json + assa.md）
│
├── voice/                        # 语音链路
...
用户输入（文字/语音）
  ...
  → 主进程 HTTP POST 到 Python 侧车 127.0.0.1:8765
    ...
    → 如需工具：policy.py 预检 → tool_call_request 事件 → 主进程 assaFinalCheck 终审 → 执行 → /tool-result 回传
...
| 双闸门安全模型 | ✅ | Python policy.py 预检 + 主进程 assaFinalCheck 终审 |
| 身份层全链路 | ✅ | assa.json → identity.py → 主进程/前端统一 |
...
| 数据目录统一收口 | 📋 草案 | 统一到 %APPDATA%\assa\，分 data/logs/cache，同上方案 |
...
| 浏览器域名白名单执行 | ❌ 未实现 | assa.json `browser.allowed_domains` 已配置，playwright_client.py 未读取校验 |
...
4. **双闸门安全**：Python policy.py 做工具权限预检，Electron 主进程做最终裁决（assaFinalCheck），执行权单一归主进程
...
Assa 侧车当前是自写的 Phase 0 精简实现（server.py 622 行），存在三个核心短板：
...
| 方案 | 做法 | 结论 |
|---|---|---|
| **A（最终选择）** | 搬入 desk-pet hermes_core 四模块（记忆/情绪/人格/时间），Assa 自写循环做定向增强 | 贴合 Assa、维护成本低、效果足够 |
...
| C | 官方 Hermes 作为独立服务运行，Assa 调 API | ❌ 失去双闸门安全控制，官方工具与 Assa 插件不兼容 |

#### 最终方案：「hermes_core 大脑 + Assa 增强循环」混合架构

**一句话**：大脑用 desk-pet hermes_core（你自己 desk-pet 项目里验证过的代码，零第三方依赖），循环和工具在 Assa 现有骨架上做定向增强，通信层和双闸门完全保留。

```
┌─────────────────────────────────────────────────────┐
│  Assa 渲染层（React 灵动岛）— 完全不变              │
├─────────────────────────────────────────────────────┤
│  Assa 主进程 — 完全不变                             │
...
| **真流式** | Ollama 调用改 stream=True，SSE 逐 token 推送，去掉当前的"8 字符/20ms 假切片" | Assa 改造 | 低（约 50 行） |
| **工具统一** | 19 个内联工具（英文命名 + `_TOOL_POLICY`）+ 16 个 schema 工具（中文命名）合并为单一 schema 定义，解决双轨制；schema 文件本身完好，无需修复编码 | Assa 改造 | 中（约 100 行） |
| **云端路由** | 侧车加模型路由层：本地 AI 自主判断任务复杂度 → 本地 Ollama 或云端 OpenAI 兼容接口；设置中显式开启（默认关闭） | 新增 | 中（约 200 行） |
| **通信/双闸门/语音/插件** | 完全保留不变 | Assa 现有 | 0 |

#### 为什么这个方案最高效

1. **大脑直接用最强的**：hermes_core 的 memory_service 是 800+ 行的完整 L0-L3 记忆系统（自动学习调度、用户画像生成、离线向量检索），比 Assa 自写的未接入版强一个量级，且零第三方依赖、是你自己 desk-pet 项目里的代码
2. **循环不重写只增强**：官方 Hermes 的 8400 行循环里 80% 是 Assa 不需要的（代码执行、终端、子代理、MoA、学习图谱），只借鉴需要的 20%（错误处理、重试、预算），增量 300 行
3. **上下文压缩自己实现精简版**：官方 8000 行里大部分是多平台/多模型适配，Assa 只需要"本地 LLM 摘要中间轮次"核心逻辑，300 行搞定
4. **双闸门和通信层完全保留**：这是 Assa 的核心安全设计和 Electron 集成，不碰就没有兼容性风险
...
- ❌ 不用官方 Hermes 的 gateway 通信层（WebSocket 多平台，Assa 用 HTTP/SSE）
- ❌ 不用官方 Hermes 的工具执行（直接执行，Assa 要双闸门终审）
- ❌ 不搬 desk-pet 的 core/pipeline.py（语音流水线，紧耦合 modules/ 第三方依赖）
- ❌ 不引入 SessionDB/FTS5（Phase 2 再说，当前 memory_service 的 SQLite 已够用）
- ❌ 不搬 desk-pet 的 interaction_agg.py（摸头/拍打/踩脚物理互动，Assa 灵动岛无此场景）
...
- **双闸门**：Assa 的安全机制——Python 侧车先做工具权限预检（gate/policy.py），Electron 主进程做最终裁决（assaFinalCheck），执行权单一归主进程
...
- **HERMES_HOME**：hermes_core 的数据目录环境变量，移植后需指向 Assa 的数据目录（%APPDATA%\assa\data\agent）

### 4.2 Electron 应用层实现

#### 主进程
- **入口** `src/main/index.ts`（1055 行）：负责窗口创建、约 25 个 `register*IpcHandlers` 注册、全局事件管理，并调用 `startAssaAgent/stopAssaAgent`；侧车 spawn 与健康巡检本体在 `src/main/services/assaAgentService.ts`（`HEALTH_INTERVAL_MS = 30_000`，健康巡检 + 自动重启）
...
- **双闸门终审**：`src/main/services/assaToolSchema.ts` 导出 `assaFinalCheck` + `assaAuditLog`，由 `ipc/agent/localToolIpc.ts` 和 `ipc/app/app.ts` 两处调用
...
6. **用户自定义上传的可替换空间 + 存储位置**：
   - 预留 `assa-avatar` 组件目录 + CSS 变量（`--assa-avatar-size`、`--assa-expression-transition` 等），汐月形象（头像/表情/动画）、主题、音效等用户可上传替换。
   - 存储位置：用户上传的自定义资源统一放在 `%APPDATA%\assa\data\custom\`（形象包/主题/音效/壁纸分子目录），与板块 2 存储规划一致。

#### v1.2 增量建议（主进程 + 渲染层）
1. **拆分对象搞错了——先拆 app.ts 不是 index.ts**：index.ts（1055 行）只是注册汇编层；app.ts 2599 行里混着工具执行且 `assaFinalCheck` 的两处调用点（`localToolIpc.ts:46`、`app.ts:384`）分散。建议先把工具执行收口成单一 `executeTool()`（预检结果 → 终审 → 执行 → 审计一条龙），板块 11 的"行为监测 + 越权拦截"才有唯一挂点。这是权限体系的地基，应排进 P0a。
...
| GET | /identity | 获取身份信息（AssaIdentityReader.as_dict） | 否 |
...
5. Electron 主进程终审（assaFinalCheck）→ 执行工具 → /tool-result 回传（超时 60s 可 env 配）
...
#### 记忆系统
- **L0 对话历史**：history.py → `history.json` 持久化（读写上限 200 条消息），侧车重启不丢；prompt 注入 `max_history_turns=10`（assa.json 可配）。正常工作
...
- **L2 长期记忆**：store.py → `memory.db`（SQLite），**Phase 0 stub，add/search/forget/count 从未被调用**，3 条手动测试数据（"用户喜欢简洁直接的沟通"等）
  ...
  - 数据路径：`DB_PATH = resolve_data_dir() / "memory.db"`，`resolve_data_dir()` 按 `ASSA_DATA_DIR` 环境变量 > 仓库 `data/` 收口；**代码中不存在 v1.0 所述的 `_DEFAULT_DB_PATH`**（该名称属于 desk-pet hermes_core 的 store.py）
...
#### 情绪系统
- **identity.py**：2 状态（calm/tired），`_infer_emotion()` 按本地时间推断（23:00-6:00 → tired，否则 assa.json `default_state=calm`），首次调用后缓存到 `_current_emotion` 不再更新；`set_current_emotion` 存在但主链路无调用。已注入 prompt（`[当前情绪：X]`）
...
- server.py 侧 `_TOOL_POLICY` 为 19 个工具声明 (meta, risks, confirm)，`_decide_tool` 的 `Ctx(current_level=...)` **自 P0a-1 起读 assa.json `security.trust_level`**（默认 1；0 = 一切需确认；≥2 因工具最高等级 2 且均标 confirm，行为与 1 等价）——L2/L3 的差异化仍待统一 schema 后定义
- 双闸门：Python 预检 → Electron 主进程 assaFinalCheck 终审 → assaAuditLog 审计（logs/assa-tools.log）
...
- 浏览器工具：Playwright 6 个（open/screenshot/navigate/click/fill/scroll），`_run_browser_tool` 路由；assa.json `browser.allowed_domains` 白名单**未被 playwright_client.py 读取校验**
...
5. **工具双轨制统一**：19 个内联工具 + 16 个 schema 工具合并为单一 schema 定义（schema 文件本身完好，重点是命名空间与权限元数据统一），server.py 启动时从 schema 加载，新工具按 schema 模板添加；同时把 assa.json `browser.allowed_domains` 真正接到 playwright_client 校验。
...
- **Assa 轻量版方案（P2）**：技能目录 `%APPDATA%\assa\data\skills\` + 技能管理设置页（展示/启用禁用/自创/导入）+ Agent 可通过工具自创技能。换脑和记忆跑通后再做
...
**迁移决策（6 项）**：
1. 记忆库用 `core.db`（hermes_core 原生命名），放 `%APPDATA%\assa\data\agent\memory\core.db`；旧 `memory.db`（3 条测试数据）废弃不迁移
...
2. 用 HEXACO 六维，UI 简化为中文性格滑块（背后映射 HEXACO 维度）；.soul 文件放 `%APPDATA%\assa\data\soul\assa.soul`
...
- **STT**：faster-whisper 1.2.1 base 模型，默认 CPU int8 推理（`ASSA_STT_DEVICE=cuda` 可切 GPU，int8_float16；GPU 推理失败自动降级 CPU 重试），模型目录 `data/models/faster-whisper-base/` 142MB；`transcribe()` 未传 `vad_filter`，即 faster-whisper 内置 Silero VAD 也未启用
...
2. **TTS**：本地 kokoro 保留（zf_xiaobei，24k）；增加 2-3 个内置音色（温柔/活泼/成熟）；用户可上传自定义音色（`%APPDATA%\assa\data\custom\voices\`）；云端 TTS 作为备选（设置显式开启）
...
5. **tmp/tts 自动清理（P0）**：TTS 生成的 wav 播放后立即删除（或保留最近 10 个，超量删最旧）。临时文件统一放 `%APPDATA%\assa\tmp\tts\`，应用退出时清理全部 tmp
...
- 记录到审计日志（`%APPDATA%\assa\logs\audit\`），与板块 10 审计体系统一
...
**9. 命名去 eisland 化（P2）**
- 统一重命名为 `assa-` 前缀，同步更新所有引用（package.json、主进程、preload、渲染层）
- 品牌一致性，不紧急但应该做
...
2. **第二闸门（Electron 主进程）**：`assaFinalCheck` 做最终裁决，执行权单一归主进程
3. **审计日志**：`assaAuditLog` 记录所有工具调用到 `logs/assa-tools.log`
...
**已知安全缺口**：
- ~~成功执行路径未审计（只有失败/拒绝有日志）~~ **✅ P0a-2a 已修复**（单一入口三种结果全审计）
- 工具定义双轨制可能导致策略覆盖不全（内联 19 工具的 `_TOOL_POLICY` 与 schema 16 工具的 policy 字段两套元数据）；**P0a-2a 补充：执行器实际实现约 90 个工具，白名单 19 个，另 70 个被终审拒绝但代码仍在**
- 主进程 ollama/customDirect 编排器无确认钩子——P0a-2a 起 confirm 工具在该路径被拒（人在环加固），但该路径若要用这些工具需补确认回调
- ~~assa.json `browser.allowed_domains` 域名白名单未被 playwright_client.py 执行~~ **✅ P0a-1 已修复**
- ~~信任等级硬编码为 1，无法由用户/设置动态调整~~ **✅ P0a-1 已修复**（`security.trust_level`，暂无设置页 UI，需手改 assa.json）
...
- 第二闸门（Electron 主进程 assaFinalCheck）：最终裁决，执行权单一归主进程
...
审计日志存 `%APPDATA%\assa\logs\audit\`，按日期分文件，保留 30 天自动清理。设置页提供"审计日志查看器"：按工具/时间/结果筛选，导出 CSV。
...
- 存 `%APPDATA%\assa\logs\runtime\`，保留 7 天自动清理
- 调试模式可提升日志详细程度（默认 info，调试时 debug）

**6. 数据安全**
- 记忆数据（core.db）和用户配置存 `%APPDATA%\assa\`，默认不加密（本地应用，系统登录已有保护）
- 提供"导出所有数据"和"清除所有数据"功能
- API Key（云端模型）加密存储在 Windows Credential Manager，不写明文配置文件
- 用户自定义上传（形象/音色/主题）存 `%APPDATA%\assa\data\custom\`，与系统数据隔离
...
3. **schema 单一事实源 + 启动一致性校验**：policy.py 与 `assaToolSchema.ts` 均从同一份 schema 加载，启动时对比工具名/等级/风险标记，不一致即拒绝启动并告警（方案级建议 B）。
...
| 6c | **执行器 90 工具 vs 白名单 19 工具**（P0a-2a 新发现） | app.ts `executeAgentLocalToolImpl` 实现约 90 个工具（eIsland 遗产：alarm/brightness/registry/service/power/win/island/todolist/file.copy/…），`ASSA_TOOL_ALLOWLIST` 只有 19 个，其余约 70 个在所有路径被终审拒绝——既是死代码（约 2000 行）也是 schema 重建的前提问题。需用户决定：删除 / 择优登记进白名单并补 Python 侧定义 | P0a-2a |
| 6d | **主进程编排器无人在环**（P0a-2a 新发现，已加固） | ollama/customDirect 编排器直接执行工具、无确认钩子；P0a-2a 起 8 个 confirm 工具在该路径被终审拒绝。若该路径仍需使用这些工具，需为编排器补确认回调（或明确该路径为"只读工具"模式） | P0a-2a |
...
| 9d | Tauri 时代 docstring 腐旧（v1.1 新增） | **P0a-1 已修 server.py 头部 + policy.py**；**P0a-2a 已修 assaToolSchema.ts 头部（"Rust 为最终权威"）**；store.py 留 P0b 随记忆替换一并处理 | v1.1 审校 |
| 9e | **既有红测试 46 项**（P0a-2a 全量跑 `src/main` 发现） | `appHelpers.test.ts` 加载期炸（electron mock ESM/CJS 命名导出）、`smtcService.test.ts` 27 项、`urlWatcher.test.ts` 15 项、`mediaHandlers.test.ts` 3 项、`captureWallpaperHandlers.test.ts` 1 项——均与本次改动零依赖。另 `localToolIpc.ts` 类型导入路径 `./types/AgentLocalToolRequest` 从未存在（P0a-2a 已修正）。仓库 tsc 另有 174 个既有错误（多为未用变量） | P0a-2a |
...
| 16 | ~~成功执行路径未审计~~ | **✅ P0a-2a 已修复**：`assaExecuteTool()` 三种结果全审计；缺口原在主进程编排器路径 | 汐月Electron版现状 |
...
| 20 | LEGAL 文档联系邮箱为上游 eIsland | Assa fork 正式发布前需评估 | 文档整理 |
...
> 执行进度：**P0a-1（侧车安全三件套 + think + 清理）已完成**；**P0a-2a（主进程收口 + 全路径审计 + deny/ask）已完成**；P0a-2b = 工具 schema 重建 + 死 IPC（需先对 70 个 eIsland 遗产工具定案）；P0a-3 = 渲染层侧（TTS 不落盘/RMS 端点检测）。

| 工作项 | 来源 | 说明 | 完成判据 | 状态 |
|---|---|---|---|---|
| 浏览器域名白名单执行 | v1.1 新增 | playwright_client 读取 assa.json `allowed_domains` 并校验（约 20 行） | 白名单外 URL 被拒并审计 | ✅ P0a-1：`_check_url()` 统一 6 入口，子域放行、前后缀伪装拒绝、仅 http/https；留空或 `*` 不限制 |
| 信任等级可配 | v1.2 新增 | `_decide_tool` 的 `Ctx(current_level=1)` 改读设置 | 设置切 L0 后写类工具被拒 | ✅ P0a-1：`security.trust_level`（assa.json，默认 1；0 = 一切需确认）；`/identity` 暴露 |
| 审计补全成功路径 | 板块 10 | 成功/失败全记录，含 token/耗时/模型来源 | 每次工具调用一条完整记录 | ✅ P0a-2a：`assaExecuteTool()` 对拒绝/成功/失败三种结果都写审计，新增 `gate`/`userConfirmed`/`source` 字段，超长字符串截断 1000 字符。此前缺口在主进程 Ollama/自定义编排器路径（只审计拒绝）。token/模型来源留真流式改造时补 |
| deny/ask 最小集 | 板块 10（v1.2 收敛） | 仅 `file.delete`、`cmd.exec` 弹确认，其余按只读/修改默认 | 破坏性工具无确认不执行 | ✅ P0a-2a：请求新增 `userConfirmed`，白名单 `confirm: true` 的 8 个工具（file.delete/cmd.exec/net.ping/browser.*）缺它即终审拒绝；渲染层两处"用户点允许"调用点已传 `true`。**行为变更**：主进程 ollama/customDirect 编排器无确认钩子，这 8 个工具在该路径上现在被拒（此前自动执行，无人在环） |
| 工具 schema 重建 | 板块 6（v1.2 方案 B） | 以主进程执行器为事实源导出 19 工具 schema，MCP `annotations` 格式，policy.py 与 assaToolSchema.ts 同源加载 + 启动一致性校验 | 两侧元数据 diff 为空 | ⏳ P0a-2b。**P0a-2a 新发现改变前提**：app.ts 执行器实际实现约 90 个工具（alarm/brightness/registry/service/power/win/island/todolist/file.copy…，eIsland 遗产），白名单只登记 19 个，其余约 70 个在所有路径上被终审按"未注册"拒绝。重建前必须先决定这 70 个的去留 |
| `executeTool()` 收口 | v1.2 新增 | app.ts / localToolIpc.ts 两处终审调用点合并为单一执行入口 | 全仓只有一处调 assaFinalCheck | ✅ P0a-2a：`assaExecuteTool(request, impl, {source})` 落在 assaToolSchema.ts；app.ts 大函数拆为 `executeAgentLocalTool`（闸门包装）+ `executeAgentLocalToolImpl`（纯实现）；localToolIpc 变薄透传。grep 验证全仓只剩闸门内部一处调用 |
...
| 死 IPC 清理 | v1.2 新增 | 删除 11 个无调用方 channel（volume-analyzer 定案后再删 `extension:*`） | — | ⏳ P0a-2b |
...
| node-gyp 预编译 + 去 eisland 化 | 板块 11（v1.2 合并） | prebuild-install，免 VS Build Tools；与包名改 `assa-` 前缀绑定为同一次破坏性变更 |
...
### 已完成（v1.1 更新状态）
1. ~~**执行项目清理**~~ ✅ 已于 commit 562a028 完成，释放约 1.35 GB（详见 `docs/Assa_Cleanup_Report_2026-09-10.md`）；跳过项：2 个被运行中 Electron 锁定的插件 DLL（约 7.3 MB），可在应用退出后补删
...
### 中期（1-2 月，体验优化）
6. **数据目录统一收口**：按方案_存储与移植规划_v1.0.md 统一到 %APPDATA%\assa\，实现 cache/logs 自动清理（解决 #5、#9）
...
### 7.2 清理清单摘要（详见 docs/Assa_Cleanup_Report_2026-09-10.md；**已于 commit 562a028 执行**）
...
| 报告 | 路径 | 覆盖范围 |
|---|---|---|
| 本报告 | docs/Assa_全面调查报告_v1.0.md（内容已更新至 v1.1） | 全项目三层综合 |
| Electron 源码调查报告 | docs/Assa_Electron_Source_Audit.md | src/main + preload + renderer + plugins（注：第 492 行"9 个 C# koffi"分类有误，见本报告 4.5） |
| Agent/语音源码调查报告 | docs/Assa_Agent_Voice_Source_Audit.md | agent/ + voice/ + data/（注：工具数 20→19、schema 17→16、"编码损坏"与"silero-vad 已装"不成立，见本报告勘误摘要） |
| 文档索引 | docs/README.md | docs/ 全部分类索引 |
| 清理清单 | docs/Assa_Cleanup_Report_2026-09-10.md | 项目 + OCR + APPDATA 垃圾扫描（已执行） |
...
| 2 | 三层架构 | ✅ 已确认 | 架构骨架不动；P0换脑+真流式；侧车巡检改事件+时间双重保险；不搬官方Hermes整套循环；hermes_core大脑+Assa增强循环 |
...
- ✅ **P0a-2a 落地**（commit 见 git log `fix(main): P0a-2a`）：`assaExecuteTool()` 单一执行入口（规范化→终审→执行→三种结果全审计，含 gate/userConfirmed/source 字段与超长截断）；app.ts 拆为闸门包装 + 纯实现；localToolIpc 变薄；deny/ask 最小集——请求携带 `userConfirmed`，8 个 confirm 工具无确认即拒，渲染层两处批准调用点已接；修正 localToolIpc 历史遗留的错误类型导入路径；新增 `src/main/services/test/assaToolSchema.test.ts`（14 项）；修稳 agentHandlers 计时抖动断言。验证：agent 目录 61/61、tsc 我改文件零错误（仓库总数 176→174）。新发现三项已入债务表 #6c/#6d/#9e

### v1.1 审校方法与对本报告的意见

**审校方法**：直接读取 package.json / server.py / identity.py / memory/*.py / voice/*.py / gate/policy.py / schemas/tool_schema.json / persona/assa.json / data/ 实际内容 / .venv pip list / git log；渲染层与插件层由两路独立扫描覆盖（hover/maxExpand/settings/states/store 类型定义；14 个插件的 csproj/binding.gyp/index.js/package.json）。IPC 数量用 `ipcMain.handle/on` 注册字符串去重统计。
...
3. 引用 hermes_core（desk-pet）的描述与 Assa 现状描述要严格分开，v1.0 把 desk-pet 的 `_DEFAULT_DB_PATH` 误写进了 Assa 记忆系统现状
...
> **报告结束**。本文档为 Assa 项目现状-目标-方法的完整基线，后续开发以此为决策依据。如需推进 P0 换脑实施，请指示。
