# Assa（汐月）Electron 应用层深度源码调查报告

> **版本**：26.7.4 ｜ **调查日期**：2026-09-10 ｜ **项目路径**：`F:\Work\Create\Assa\Assa`
...
│   │   ├── services/                  # [已查证] 服务层：appLifecycle/updater/hotkey/assaAgent/captureOcr/nativeCapture/chromiumFlags/imageTranslation
│   │   ├── core/                      # [已查证] 核心模块
...
| Channel | 类型 | 功能 | 文件 |
|---|---|---|---|
| `assa:health` | handle | 汐月Hermes侧车健康检查 | assaAgentIpc.ts:54 |
| `assa:chat` | handle | 文本对话（→Python侧车） | assaAgentIpc.ts:62 |
| `assa:voice` | handle | 语音对话（录音→STT→LLM→TTS） | assaAgentIpc.ts:70 |
| `assa:transcribe` | handle | 本地音频转写（16k mono wav base64） | assaAgentIpc.ts:79 |
| `assa:stream:start` | handle | 启动工具化流式对话（SSE） | assaAgentIpc.ts:88 |
| `assa:stream:abort` | handle | 中止流式会话 | assaAgentIpc.ts:155 |
| `assa:tool-result` | handle | 回传本地工具执行结果 | assaAgentIpc.ts:163 |
| `assa:identity` | handle | 获取汐月身份配置（name/role/model/情感/记忆/工具等） | assaIdentity.ts:91 |
...
| **hover** | 500×60 | 30000 | 禁用 | 多标签页：time(控制中心)/lyrics/weather/assa(汐月Agent)/pomodoro/custom(自定义URL页) |
...
- appId: `com.assa.app`，productName: `汐月`
...
| # | 问题 | 严重度 | 位置/证据 |
|---|---|---|---|
| 1 | **插件命名不一致**：项目已改名 assa，但14个插件仍保留 `eisland-windows-*` 前缀，`appUserModelId` 仍为 `com.eisland.app`（index.ts:862），store目录仍为 `eIsland_store`（index.ts:573），自定义协议仍为 `eisland-media://` / `eisland-qishui://` | 中 | 全局历史遗留 |
...
| 4 | **Python 侧车进程管理**：`assaAgentService` 通过 child_process 拉起 Python server.py，崩溃重启策略未在调查中确认 | README 描述 + index.ts:865 |
