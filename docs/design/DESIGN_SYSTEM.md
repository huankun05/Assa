...
> idle 态：**默认不因悬停而抢焦点**。展开以**点击**或**明确热区+停留**为主。

**落地阶梯（已定）**：

| 阶段 | 方案 | 状态 |
|---|---|---|
| **P0** | 默认悬停展开；启用 enterDelay（250ms）；**右键让路约 3 秒**（临时隐藏，计时内不回弹） | **已实现** |
| **P1** | idle 热区收窄（可选加强） | 可选 |
| **P2** | 贴顶休眠：只露 8–12px 露头条 | 可选 |

**遮挡策略（已定，2026-09-12）**：

> 不以「强制点击展开」为默认（会失去灵动岛感）。  
> **悬停展开 + 右键让路 3 秒**：让路期间岛不可见、不抢悬停；长期隐藏走设置/热键。  
> 发现性：首次引导「交互」卡 + 行为设置文案说明。
...
| 项 | 内容 |
|---|---|
| 素材 | P0 七态入库 `public/image/agent/` |
| 接线 | PHASE_IMAGE / AVATAR / 阶段文案 i18n 统一表 |
| 归档 | 恐龙移 legacy；删默认引用 |
| 图标 | 托盘 16 + 应用 256 替换 |
| 过渡 | 缺图 mood 共用已有少女图 |
...
| 隐私安全 | 分区与密钥入口归类 |
| 按钮并轨 | hotkey → 二套 |
| 浅色 | chip/录制/碟片/loading/下拉公共类 |
| 大面板设置 | 打开独立窗 |

**验收**：安全项一处可寻；无幽灵 class；浅色无明显翻车点。
