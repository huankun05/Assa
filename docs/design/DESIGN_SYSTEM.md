...
| 优先级 | 事项 | 依赖 |
|---|---|---|
| 1 | **番茄钟等 hover 视觉精修**（当前仅功能可用，设计待打磨） | 无 |
| 2 | **品牌图标**（托盘/应用 ICO） | `CHARACTER_ASSET_CHECKLIST` §6B 出图 |
| 3 | 引导页全文化（eIsland→汐月、logo、交互演示） | 无 / 可与 2 并行 |
| 4 | 设置 IA 大类重划 + 隐私分区深化 | 独立里程碑 |
| 5 | Lucide 增量、浅色扫尾 | 可穿插 |

**已取消**：P2 屏顶露头条（遮挡方案「悬停 + 右键让路」已验收）。
