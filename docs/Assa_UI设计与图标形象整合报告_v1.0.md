# Assa UI 设计与图标形象整合报告 v1.0

> 整合日期：2026-09-11
> 整合来源：`Assa_设计与改进建议_v1.0.md` + `Assa_图标与形象设计调查报告.md`
...
| 源文件尺寸 | 用途 | 显示尺寸 | 位置 |
|---|---|---|---|
| 512×512 PNG | 所有头像源文件（统一） | 44px | 灵动岛 hover 态头像（AssaTab） |
...
3. `assaMoodConfig.ts` 扩展为 7 种状态，全部配置图片路径
4. CSS 微动系统 P0 已实现（呼吸感 + 7 种状态独特微动 + 状态点 7 色）
5. `AssaAvatar.tsx` 组件支持 mood class，SVG 兜底扩展为 7 种表情

📋 **待后续**：
1. `assets/avatar/T.jpg`（139.6KB）用途不明，需确认是否为临时占位
2. `agent/persona/assa.md` 中"无形象，靠声音+状态点"的表述已落后，需更新
...
实现位置：`src/renderer/styles/hover/assa-tab.css` + `AssaAvatar.tsx`（mood class）

| 动态效果 | 实现方式 | 参数 | 效果 |
|---|---|---|---|
| **呼吸感** | `@keyframes assa-breathe`，`scale(1→1.015→1)` | 4秒周期，ease-in-out，无限循环 | 最基础的"活着"信号 |
| **情绪切换过渡** | `.switching` 类：`opacity 0→1` + `translateY(2px)` + `scale(0.98)` | 300ms 过渡 | 避免硬切的割裂感（JS 控制加类） |
| **calm 待机** | `assa-calm`：`scale + translateY(1px)` 浮动 | 6秒周期，更慵懒 | 安静待机 |
| **happy 开心** | `assa-happy`：`scale(1.02) + translateY(-2px)` 弹性跳 | 2秒周期 | 活泼开心 |
| **thinking 思考** | `assa-thinking`：`rotate(-1.2°→+1.2°)` 左右轻晃 | 3秒周期 | 正在琢磨 |
| **tool 操作** | `assa-tool`：`rotate(-0.8°)` 极轻微点头 | 2.5秒周期 | 专注操作 |
| **listening 聆听** | `assa-listening`：`rotate(3°→2°)` 侧倾保持+微点头 | 2秒周期 | 认真在听 |
| **confuse 困惑** | `assa-confuse`：`rotate(-1.5°→+1.5°)` 快速小幅度抖动 | 1.5秒周期 | 迷茫"啊？" |
| **speaking 说话** | `assa-speaking`：`scale(1.015) + translateY(-1px)` 快节奏跳 | 0.45秒周期 | 模拟说话节奏 |
...
1. 设计 Assa 自己的应用图标（基于汐月角色形象 或 抽象"月"元素）
...
| 灵动岛 hover assa 页 | 2+头像 | 麦克风/对话气泡 + AssaAvatar |
| 设置页 | 30+ | 各设置项图标 |
...
- 用户可自定义壁纸（存储在 `%APPDATA%\assa\data\custom\wallpaper\`）
...
**Lucide 推荐理由**：ISC 许可宽松，风格与当前 AssaAvatar/AssaActionIcons 的线性风格一致（round cap/join），1000+ 图标覆盖绝大部分 UI 需求。
...
6. **保留现有自定义图标**：AssaActionIcons（麦克风/对话气泡）风格已与 Lucide 一致，可保留或替换为 Lucide 对应图标

#### Lucide 风格样例（已展示确认）

播放/暂停/上一曲/下一曲、截图/计算器/翻译/设置、亮度/音量/音乐/麦克风、AI助手/对话/爱心/下载 共 16 个图标，风格与现有 AssaActionIcons 一致，替换后无割裂感。
