# Assa（汐月）Agent 侧车与语音链路 · 深度源码调查报告

> 调查日期：2026-09-10 | 项目路径：`F:\Work\Create\Assa\Assa`
...
┌─────────────────────────────────────────────────────────────┐
│                    Electron 主进程                            │
│  assaAgentService.ts  ── spawn ──►  python agent/server.py │
│  (健康巡检 30s / 自动重启 max3)       (127.0.0.1:8765)     │
│                                                               │
│  assaAgentIpc.ts ── HTTP fetch ──►  /health /chat /voice  │
│                       ── SSE stream ─► /chat/stream          │
│                       ── HTTP POST ──► /tool-result          │
└──────────────┬──────────────────────────────────────────────┘
               │ IPC (assa:stream:event:<sid>)
               ▼
┌─────────────────────────────────────────────────────────────┐
│                  Electron 渲染进程                            │
│  assaLocalAgent.ts  ── 消费 SSE 事件 ──► UI 渲染 + TTS 播放 │
...
Python 侧车内部组件：
  ...
  persona/assa.json + assa.md ── 人格设定
  main.py            ── Phase0 stub（968B，未被 server.py 使用）
```

**关键事实 [已查证]：**
- HTTP 服务绑定 `127.0.0.1:8765`（`ASSA_AGENT_PORT` 可覆盖）
...
| GET | `/identity` | 读取完整身份信息 | — | `AssaIdentityReader.as_dict()`（name/role/model/emotion/memory/tools 等 20+ 字段） |
...
- `_infer_emotion()`（identity.py:179-188）：只看当前小时，23-6点 → `tired`，否则 → `calm`（从 assa.json 的 `emotion.default_state` 读取）
...
### 7.1 配置文件（`persona/assa.json`）[已查证]
...
[身份标识] 你是 汐月（Assa），本地常驻 AI 管家。
[系统标识] assa-local-agent
...
3. window.api.assaTranscribe(audio_b64)
   → IPC assa:transcribe → HTTP POST 127.0.0.1:8765/transcribe
...
- 默认设备：**CPU**（`ASSA_STT_DEVICE` 可设为 `cuda`）
...
| 项目 | 值 |
|------|-----|
| 模型 | `qwen3-4b-32k`（默认，`ASSA_LLM_MODEL` 或 assa.json `model_default` 可覆盖） |
...
**[已查证]** Electron 主进程启动侧车时会注入环境变量 `ASSA_LLM_MODEL`（默认 `qwen3-4b-32k`）。

---

## 10. 与 Electron 的通信桥接

### 10.1 侧车生命周期管理（`assaAgentService.ts`）[已查证]

```
启动: startAssaAgent()
  ├─ 幂等检查（已在运行则忽略）
  ├─ 解析 Python: ASSA_PYTHON 环境变量 > .venv/Scripts/python.exe > PATH python
  ├─ spawn(python, [agent/server.py], {cwd: app.getAppPath(), env: {ASSA_AGENT_PORT, ASSA_LLM_MODEL}})
  ├─ stdout/stderr 转发到 Electron 控制台日志（前缀 [assa-agent] / [assa-agent:err]）
  ├─ exit 事件：非零退出码 → scheduleRestart()（2s 后重启，最多 3 次）
  └─ startHealthLoop()：每 30s GET /health，5s 超时

健康巡检: healthLoop()
  ├─ 若进程在运行 → fetch /health
  ├─ 失败 → stopAssaAgent() + scheduleRestart()
  └─ 成功 → cancelScheduledRestart()（重置重启计数）

停止: stopAssaAgent()（应用退出时调用）
  ├─ stopHealthLoop()
  ├─ cancelScheduledRestart()
  └─ child.kill()
```

### 10.2 IPC 桥接（`assaAgentIpc.ts`）[已查证]

| IPC 通道 | 方向 | 转发到 | 说明 |
|----------|------|--------|------|
| `assa:health` | Renderer→Main→HTTP | GET /health | 健康检查 |
| `assa:chat` | Renderer→Main→HTTP | POST /chat | 文本对话（非流式） |
| `assa:voice` | Renderer→Main→HTTP | POST /voice | 服务端录音全链路 |
| `assa:transcribe` | Renderer→Main→HTTP | POST /transcribe | 录音 base64 → 文本 |
| `assa:stream:start` | Renderer→Main→SSE | POST /chat/stream | **SSE 流式对话**，主进程逐事件通过 `assa:stream:event:<sid>` 推回渲染层 |
| `assa:stream:abort` | Renderer→Main | — | 中止流式会话（AbortController） |
| `assa:tool-result` | Renderer→Main→HTTP | POST /tool-result | 工具执行结果回传 |
...
- 5 分钟 SSE 读取超时
- `AbortController` 支持中止（`assa:stream:abort`）

### 10.3 渲染层消费（`assaLocalAgent.ts`）[已查证]

`streamAssaAgent({message, signal, onEvent})`：
1. 生成 sessionId，注册 `assa:stream:event:<sid>` 监听
2. 调用 `window.api.assaStreamChatStart(sid, message)`
...
- **tsc 既有错误**：5 个报错在 `assaLocalAgent.ts` / `assaIdentity.ts`，属未触碰文件的既有问题
...
- 录音 → PCM16 累积 → 拼 wav → `window.api.assaTranscribe`（faster-whisper 本地转写），**替代原腾讯实时语音识别（已删除）**
...
| 10 | **browser.allowed_domains 白名单未执行** | `persona/assa.json` vs `playwright_client.py` | assa.json 定义了白名单但 playwright_client.py 注释明确说"不预设域名白名单"，配置与实现不一致 |
...
| 16 | `store.py` 数据路径硬编码 | 注释提到 `_DEFAULT_DB_PATH` 硬编码相对路径是已知待改造点；实际已用 `resolve_data_dir()` 收口，但 `resolve_data_dir()` 开发期默认指向 repo 内 `data/`，打包后需 Rust 注入 `%APPDATA%/assa` |
| 17 | tsc 既有错误 | `assaLocalAgent.ts` 和 `assaIdentity.ts` 有 5 个类型错误，属历史遗留 |
...
| HTTP 端口 | 8765 | `server.py` + `assaAgentService.ts` |
| LLM 模型 | qwen3-4b-32k | `assa.json` + 环境变量 |
| 健康巡检间隔 | 30s | `assaAgentService.ts` |
| 最大重启次数 | 3 | `assaAgentService.ts` |
| 工具调用最大轮数 | 8 | `server.py _MAX_TOOL_ROUNDS` |
| 工具结果超时 | 60s | `server.py _TOOL_RESULT_TIMEOUT_S` |
| 对话历史上限 | 200 条 | `history.py _MAX_MESSAGES` |
| 上下文轮数 | 10 轮 | `assa.json memory.max_history_turns` |
