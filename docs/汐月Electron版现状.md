# 汐月 Assa —— Electron 版现状（基线 2026-08-31，更新至 2026-09-02）
...
Electron 主进程（src/main）
 ...
 │     HTTP 127.0.0.1:8765（ASSA_AGENT_PORT 可覆盖）
 │     ├─ GET  /health         {ok, model}
 │     ├─ POST /chat {text}    → Ollama(qwen3-4b-32k) + persona/assa.md + 历史≤10轮 → kokoro TTS
 ...
 ├─ IPC 桥（ipc/agent/assaAgentIpc.ts）：assa:health/chat/voice/transcribe
 ├─ 主进程工具终审（services/assaToolSchema.ts）：assaFinalCheck + assaAuditLog（落盘 logs/assa-tools.log）
 │     └─ 入口：ipc/app/app.ts executeAgentLocalTool（SSE 工具调用与本地工具 IPC 的统一入口，顶部即终审）
 └─ preload：window.api.assaHealth/assaChat/assaVoice/assaTranscribe/assaStreamChatStart/executeAgentLocalTool

渲染进程（eIsland React UI）
 ├─ 原生 Agent（states/agent + maxExpand AiChatTab）：useAgentRunner/useChatSend 只跑汐月
 │     api/ai/assaLocalAgent.ts：window.api.assaStreamChatStart → 原生 SSE 事件流（think→chunk→final）+ TTS 自动播放
 │     api/ai/assaLocalTool.ts：工具结果经 window.api.assaToolResult 回传侧车（替代原 mihtnelis 平台回传）
 ├─ 语音球（agentVoiceInput）：录音 → PCM 累积 → wav → assaTranscribe（本地 faster-whisper）
 ├─ 身份层（主进程/渲染进程）：
 │     agent/persona/assa.json：程序化身份事实源（名字/模型/语言/情绪/记忆/工具参数）
 │     agent/identity.py：统一身份接口（get_name/get_visible_name/get_model_default/build_system_prompt）
 │     src/main/ipc/agent/assaIdentity.ts + src/main/window/mainWindow.ts + src/main/tray.ts：身份读接入
 │     src/preload/index.ts：暴露 window.api.assaIdentity()
...
| 层 | 文件 | 说明 |
|---|---|---|
| 侧车 | `agent/persona/assa.md` | 汐月人设（system prompt） |
| 侧车 | `agent/persona/assa.json` | 程序化身份配置（名字/模型/语言/情绪/记忆/工具参数） |
...
| 主进程 | `services/assaAgentService.ts` | 侧车生命周期（spawn/kill） |
| 主进程 | `ipc/agent/assaAgentIpc.ts` | 四端点 IPC 转发 |
| 主进程 | `ipc/agent/assaIdentity.ts` | 身份 IPC 处理（getIdentity/invalidateIdentityCache） |
| 主进程 | `services/assaToolSchema.ts` | 工具白名单 + 风险等级 + 终审闸门 + 审计落盘 |
...
| 渲染 | `api/ai/assaLocalAgent.ts` | SSE 事件流包装 + TTS 播放 |
...
| 渲染 | `utils/assaIdentity.ts` | 渲染进程身份缓存工具 |
...
（`executeAgentLocalTool` 顶部 `assaFinalCheck`，拒绝未注册/空工作区高风险工具，拒绝事件审计落盘）；
**身份层全链路接入**（`assa.json` → `identity.py` → server.py/主进程/前端统一读取，改配置即可改名字/模型）；
...
**前端身份显示接入**（`AssaTab.tsx` 从 `assaIdentity.ts` 读取可见名，同步缓存+异步更新，改配置即可改 UI 显示的名字）；
...
+ 5 类型文件 + 测试，仅保留活着的 `resolveMihtnelisWebAccess` 网页授权解析）；i18n `common.appName` → 汐月/Assa；
...
## 四之一、长截图（滚动拼接）
功能已基本可用，详细问题与根因见 **`docs/长截图问题排查记录.md`**。截至 r50b：

**已完成核心改进**：
- r49：从方案A（依赖鼠标在选区内透传）升级到**方案B PostMessage WM_MOUSEWHEEL** → 彻底消除鼠标位置依赖，鼠标移出选区也能正常滚动、且不再抢鼠标位置，彻底解决"抢鼠标/鼠标被重置"问题。
- r49 方案B支持 Chromium 系直接命中 `Chrome_RenderWidgetHostHWND` 渲染子窗口 → delta 与像素位移线性精准，且与前台焦点无关，任意窗口当前台依然滚动。
- r50 尝试修复 r49 后复发的「大部分内容重复+清晰度差」，但收紧静止判定+丢弃 soft_frame 致极慢+丢内容 → r50b 已回退两处，保留 delta 自适应 clamp 防 120↔420 震荡，软帧仅落盘不丢弃（保真正常静帧）、静止判定回退原值恢复速度。

**核心问题**：三类早期主症状（画质模糊、接缝横线、蓝色边框残留）均已在代码层修复；方案B 鼠标依赖问题彻底解决。当前唯一待解决：Chromium 平滑滚动异步推进导致位移漂移（首格 120px→重叠 76%→重复囤积），需基于实测位移设计更稳定的滚轮 delta 反馈，仍待后续。
