...
功能已基本可用，详细问题与根因见 **`docs/长截图问题排查记录.md`**。截至 r38：
画质模糊（编辑器 resize 栅格尺度失效）、接缝横线（DP 最小能量缝替代羽化带）、蓝色边框残留（CSS 去蓝边 + 编辑态禁手柄）
三类主症状代码层均已修复；拼接几何在 r26 高选区与 dwarf 矮选区真实帧 E2E 上总高精确命中。
**残留风险**：矮选区重叠下限偏紧（≈11% 帧高）、周期内容假谷在极低噪声页仍可能锁错、首格固定 delta 已在 r38 回退为 -60。
r38 之后尚未经用户实测闭环，待下一轮截图反馈。
