...
> 目标：摸清哪些地方需要图标/头像，规划一整套属于 Xiyue 自己的图标体系和 AI 形象
...
1. 设计 Xiyue 自己的应用图标（建议基于"汐月"=潮汐+月亮的意象，或基于汐月角色形象）
...
5. **重命名**：`eisland-icon.ts` → `xiyue-icon.ts`（P2，涉及大量引用修改）
...
| 组件 | 位置 | 说明 |
|---|---|---|
| XiyueAvatar.tsx | hover/pages/xiyue/components/ | 内联 SVG 头像，圆形框+眼嘴表情+AI sparkle |
| xiyueMoodConfig.ts | hover/pages/xiyue/config/ | 4 种状态（happy/thinking/confuse/listening），AVATAR_IMAGE_MAP 全空（用 SVG 模式） |
...
- **用户自定义**：`%APPDATA%\xiyue\data\custom\avatar\`（板块 5 已确认的自定义上传）
...
| xiyue（AI） | 2 | MicrophoneIcon(自定义), ChatBubbleIcon(自定义) + XiyueAvatar |
...
- 用户可自定义壁纸（板块 5 已确认，存储在 `%APPDATA%\xiyue\data\custom\wallpaper\`）
...
**Lucide 推荐理由**：ISC 许可宽松（可商用，只需保留版权声明），风格与当前 XiyueAvatar/XiyueActionIcons 的线性风格一致（round cap/join），1000+ 图标覆盖绝大部分 UI 需求，社区活跃更新频繁。
