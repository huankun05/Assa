...
function callHelper(args, timeout = 10000) {
  ...
  if (!helperPath) {
    console.error(
      '[xiyue-windows-hardware-info-helper] Helper executable not found. Searched candidates:',
      helperCandidates,
    );
    return null;
  }
  ...
  if (result.error) {
    console.error(
      '[xiyue-windows-hardware-info-helper] Failed to spawn helper process',
  ...
  if (result.status !== 0) {
    console.error(
      '[xiyue-windows-hardware-info-helper] Helper exited with non-zero status',
  ...
  if (!result.stdout) {
    console.error(
      '[xiyue-windows-hardware-info-helper] Helper produced no stdout',
      { helperPath, args, status: result.status, signal: result.signal, stderr: result.stderr },
    );
    return null;
  }
  ...
  try {
    ...
    if (!Array.isArray(parsed)) {
      console.error(
        '[xiyue-windows-hardware-info-helper] Helper returned non-array JSON payload',
    ...
    console.error(
      '[xiyue-windows-hardware-info-helper] Failed to parse JSON from helper stdout',
